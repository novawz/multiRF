...
#'
#' Aggregates the per-node split statistics that the native C++ engine stores
...
#'
#' @param mod A single fitted forest model from the native engine (with
...
cluster_weighted_imd <- function(mod, cluster, normalized = TRUE) {
  ...
  if (!is.list(tree_info) || length(tree_info) == 0L) {
    stop("`mod` has no `tree_info`; cluster_weighted_imd() requires a native-engine fit.")
  }
  if (is.null(tree_info[[1L]]$imd_x_score)) {
    stop("`mod$tree_info` does not carry `imd_x_score`; refit with the native engine.")
  }
